...
const DB_COLLECTORS = [
    ...
    ['expected_controllers', async () => {
        // ⚠️ [042] ИСПРАВЛЕНО: прежняя редакция этого комментария винила
        // `DEFAULT 'active'` у контроллеров со ссылкой на 01_init_database.sql:120.
        // Строка 120 — это `transformers`, где 'active' законен. У `controllers`
        // DEFAULT'а нет вовсе, а домен теперь закреплён CHECK'ом (миграция 042);
        // сид'овые строки со статусом 'active' переведены в 'online'.
        //
        // Счёт от ОБРАТНОГО тем не менее сохранён, и это не наследие: по смыслу
        // ждать телеметрию следует от всего, что не выведено в обслуживание, —
        // включая 'offline'. Молчащий контроллер и есть авария, которую правило
        // обязано увидеть; фильтр по 'online' её бы как раз спрятал.
        //
