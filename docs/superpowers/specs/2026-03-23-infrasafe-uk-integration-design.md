...
**Resolving `building_ids`:** The current `infrastructure_alerts` table stores only `affected_buildings` (integer count), not specific IDs. The webhook sender in `alertService.sendNotifications()` must resolve building IDs by querying the infrastructure relationship:
- For transformers: query `power_transformers` → get connected buildings via `buildings` table where `transformer_id` matches
- For controllers: query `controllers` → get `building_id`
...
4. If rule found and enabled:
   ...
   b. For each building: call UK `POST /api/v2/requests/create` with mapped category + urgency
   c. Store one mapping row per request in alert_request_map
...
1. **DB migration:** Add `external_id UUID` column to `buildings` table
2. **alertService.js:** Implement `sendWebhookNotification()` — POST alert events to Bridge
3. **buildings API:** Accept `external_id` in create/update endpoints
4. **Frontend (script.js):** New "Заявки UK" layer with badge markers and popup section
5. **Frontend (map-layers-control.js):** Add layer toggle
6. **Docker Compose:** Add bridge service container
