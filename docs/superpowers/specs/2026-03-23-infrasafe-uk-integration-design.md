...
CREATE TABLE building_map (
    ...
    uk_building_id  INTEGER NOT NULL,
    infrasafe_building_id INTEGER,       -- NULL until created in InfraSafe
...
| `floors`, `entrances`, `apartments_count` | UK | Synced from UK building data if available |
| `year_built`, `has_hot_water` | InfraSafe | Infrastructure-specific, set by InfraSafe admin |
| `controllers`, `metrics`, `alerts` | InfraSafe | InfraSafe-only entities, never synced |

**On `building.updated` from UK:** Bridge updates only UK-owned fields in InfraSafe. InfraSafe-owned fields (coordinates, region, etc.) are preserved. The Bridge update call uses `PATCH` semantics — only specified fields are changed.
...
| --- | --- | --- |
| `transformer` | `buildings` WHERE `primary_transformer_id = :infrastructure_id` OR `backup_transformer_id = :infrastructure_id` | Buildings reference transformers via two FK columns in `buildings` table (`Building.js:161`) |
| `controller` | `controllers` WHERE `id = :infrastructure_id` → `building_id` | Direct FK, always one building |
| `water_source` | `cold_water_sources` WHERE `id = :infrastructure_id` → related buildings via `water_lines` | Indirect through water line topology |
| `heat_source` | `heat_sources` WHERE `id = :infrastructure_id` → related buildings via district heating network | Indirect, may need join through infrastructure tables |

**Note:** `power_transformers` in analytics (`analyticsService.js`) is the same entity as referenced by `buildings.primary_transformer_id` / `buildings.backup_transformer_id`. The ID space is shared.
...
4. **buildingController.js:** Support PATCH semantics for Bridge updates (only update provided fields, preserve others)
