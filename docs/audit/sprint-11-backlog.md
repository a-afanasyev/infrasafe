...
- **SEC-20 · ✅ CLOSED код (Phase E / PR-E2, 2026-06-07) · MEDIUM · нет rate-limit на nginx-слое** — `nginx-config/nginx.production.conf` (нет
...
- **SEC-34 · LOW/INFO · hardening-пачка** (бывшие L-1..L-13 + INFO):
  ...
  | g | ✅ CLOSED код (PR-E2, 2026-06-07) — `location = /api-docs` + `^~ /api-docs/` → `return 404` (обе формы) | `nginx-config/nginx.production.conf` |
