...
| **SEC-34h** JWT_2FA_SECRET | **✅ CODE DONE (TDD, suite green) — deploy pending** — `authService` подписывает/верифицирует 2FA temp-token отдельным `JWT_2FA_SECRET` (dev/test → fallback на `JWT_SECRET`); prod-валидация в `env.js` (`PRODUCTION_REQUIRED_VARS` + проверка неравенства ≠ `JWT_SECRET`). Rollout env-first: оператор добавляет `JWT_2FA_SECRET` в `.env.prod` → затем rebuild/deploy образа | Deploy отдельным авторизуемым шагом |
