...
| SEC-34e **L-5 ✅ / L-6 deferred** | **L-5 DONE 2026-06-14**: `img-src` сужен с blanket `https:` до `'self' data: https://*.tile.openstreetmap.org https://*.tile.opentopomap.org` в helmet (`server.js`) + edge `:230`/`:416` (наш surface; `/uk/` `:329` не тронут — surface УК). Оба tile-хоста (OSM base + OpenTopoMap relief) сохранены. **L-6 (убрать `style` из DOMPurify ALLOWED_ATTR) — DEFERRED**: попапы карты несут 62 inline `style=`; удаление сломает вёрстку, нужен tracked CSS-рефактор (`server.js:49-50`); ценность low (CSP несёт `style-src 'unsafe-inline'`). | L-6 → с CSS-рефактором |
