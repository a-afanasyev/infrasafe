...
### B-024 — Map layer counters show `(0)` for anonymous visitors despite rendered markers

**Найдено 2026-05-31** при браузерном QA прода (post-B-012 deploy; **НЕ регрессия** — деплой
`3c23225..26f206f` тронул только тестовые файлы, ноль изменений в `public/`/JS).

**Что**. На публичной карте (`/`, аноним) маркеры зданий/контроллеров **отрисовываются** (данные из
публичного `GET /api/buildings-metrics` → 200), но счётчики в панели «Объекты инфраструктуры» показывают
`🏢 Здания (0)`, `📊 Контроллеры (0)`, `⚠️ Алерты (0)` и т.д. для всех слоёв.

**Почему так**. Маркеры и счётчики берутся из **разных** источников:
- маркеры → `/api/buildings-metrics` (public allowlist, anon получает усечённые данные) → 200;
- счётчики слоёв → отдельные auth-gated эндпоинты (`/api/buildings`, `/api/controllers`,
  `/api/transformers`, `/api/alerts`, …) → **401 для анонима** (default-deny).

Проверено curl'ом: все четыре эндпоинта → 401 для anon, `buildings-metrics` → 200. То есть `(0)` —
это **корректное** поведение каждого отдельного fetch'а (401 → пустой массив → count 0), но на UI
получается несогласованность: «маркеры есть, а счётчик 0». Для **авторизованного** пользователя счётчики
должны заполняться (подтвердить в admin-QA — см. ниже).

**Severity**. Низкая, cosmetic, **anon-only**. Не теряются данные, карта функциональна. Но вводит в
заблуждение: выглядит как «слой пустой», хотя объекты на карте видны.

**Fix-варианты** (выбрать при взятии в работу):
1. Для anon считать счётчики из того же `buildings-metrics` payload'а, что и маркеры (консистентность
   источника) — минимальный, но покрывает только здания/контроллеры (что есть в metrics).
2. Скрывать счётчик `(N)` целиком, когда соответствующий fetch вернул 401 (показывать просто название
   слоя без числа для анонима) — честнее отражает «данные не загружены из-за прав».
3. Добавить публичные count-only эндпоинты (агрегаты без чувствительных полей) — больше работы, но даёт
   реальные числа анониму.

**Trigger**. Косметика; брать когда будет проход по map-UX, либо вместе с `feature/frontend-redesign`
merge (B-008), где карта переписывается.

**Estimate**. ~1-2 часа (вариант 1 или 2) + dev smoke anon/auth.
