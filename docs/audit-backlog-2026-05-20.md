...
>
> **Sprint 8 — P1-14 architectural epic: split ukIntegrationService — 2026-05-22**: `feat/refactor-p1-14-split-uk-integration`. Closed `[P1-14]` god-class. 752 LoC monolith split into 5 single-responsibility modules under `src/services/uk/`: `configProxy.js` (140 LoC), `webhookVerifier.js` (200 LoC), `buildingSync.js` (155 LoC), `alertForwarder.js` (200 LoC), `requestProcessor.js` (130 LoC). `ukIntegrationService.js` itself dropped 752 → 93 LoC, now a facade with bound-method re-exports + property proxies (`_seenSignatures`, `_requestCountsCache*`) so existing route handlers, jest mocks, and 1983 tests pass without modification. Only 1 test file needed a 2-line spy retarget. Cross-module calls explicit: `alertForwarder → configProxy.isEnabled`, `alertForwarder → webhookVerifier.logEvent`, `requestProcessor → configProxy.invalidateRequestCache`. Listener `ALERT_CREATED` migrated into `alertForwarder.js`; `UK_REQUEST_RESOLVED` emit stays inside `requestProcessor.js`. Tests: 99 suites / 1983 ✓ unchanged baseline.
>
...
- **Status**: `fixed` — Sprint 8 (2026-05-22). Actual size at split time was **752 LoC**. Split into 5 modules under `src/services/uk/`: `configProxy.js` (isEnabled/getConfig/updateConfig + UK API proxy + 60s cache), `webhookVerifier.js` (HMAC + replay protection + logEvent/isDuplicateEvent), `buildingSync.js` (handleBuildingWebhook + _generateExternalId), `alertForwarder.js` (sendAlertToUK + resolveBuildingIds + owns ALERT_CREATED listener), `requestProcessor.js` (handleRequestWebhook + emits UK_REQUEST_RESOLVED). `ukIntegrationService.js` itself is now a 93-line facade re-exporting the public API with bound methods + property proxies (`_seenSignatures`, `_requestCountsCache*`) for backward-compat with existing tests. All 1983 tests still pass — facade design means routes and existing mocks need zero changes.
