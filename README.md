...
[A-26] Цепочка описывает **направление зависимостей**, а не запрет: она о том,
кто кого зовёт, и НЕ означает, что весь SQL живёт в моделях. Repository-слоя в
проекте нет и не планируется — вводить его ради соответствия схеме дороже, чем
назвать отступления. Их три, и у каждого есть владелец:

| Отступление | Где | Почему так |
|---|---|---|
| Админ-контроллеры ходят в БД мимо модели | восемь `src/controllers/admin/*Controller.js` через `buildPaginatedList(pool, …)` | Это не «свой SQL», а один генератор списков с whitelist-валидацией идентификаторов (`src/utils/adminQueryBuilder.js`). Восемь моделей-обёрток вокруг одного вызова дали бы восемь мест для расхождения фильтров и сортировок — а не слой. |
| SQL живёт и в сервисах | `alertService` + `src/services/alert/*`, `analyticsService`, `powerAnalyticsService`, `buildingMetricsService`, `src/services/uk/*` | Запросы агрегирующие: оконные функции, материализованные представления, CTE поверх нескольких таблиц. Такой запрос принадлежит сценарию, а не сущности; разложенный по моделям, он распадается на несколько round-trip'ов и теряет смысл. Владелец запроса — сервис, чьё имя стоит в файле. |
| Модель зависит от сервиса | `src/models/User.js` → `cacheService` | Инвалидация кэша пользователя обязана происходить в той же операции, что и запись; вынесенная в вызывающий код, она разъезжается с ней при первом же новом вызове. Граница «весь SQL по `users` — только здесь» сторожится тестом (AR-3). |

Чем эти отступления **не** являются: ad-hoc SQL в контроллере мимо
`adminQueryBuilder`, запросы по `users` вне `User.js`, обращение к пулу из
маршрутов. Взятие соединения ради `pg_try_advisory_lock` (drain-воркер УК,
сверяющий проход A-03, верификация алертов) запросом к данным не является —
это управление сессией.
