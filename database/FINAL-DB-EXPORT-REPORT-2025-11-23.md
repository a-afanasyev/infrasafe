...
> ⚠️ **Снапшот от 23.11.2025, устарел.** Файл
> `database/export/schema_export_20251123_124422.sql`, на который ссылается
> отчёт ниже, удалён 2026-08-05 (DE-3): он описывал схему ДО миграций 011–041 и
> при чтении выдавал себя за актуальную. Источник правды по схеме —
> `database/init/01_init_database.sql` плюс применённые миграции
> (`database/migrations/`, состояние — в таблице `schema_migrations`). Отчёт
> оставлен как исторический документ.
