...
# A-03 (аудит 08.09.2026) — сверяющий проход по открытым алертам без заявки в УК
UK_INTENT_RECONCILE_ENABLED=true  # РУБИЛЬНИК, а не opt-in: по умолчанию включён.
                           # Проход закрывает окно между коммитом алерта и
                           # слушателем ALERT_CREATED (падение между ними
                           # оставляло открытую аварию без тикета навсегда).
                           # Выключенный по умолчанию механизм починки — ровно
                           # тот способ, которым здесь однажды завёлся мёртвый
                           # код (AUD-001), поэтому безопасность даёт не флаг, а
                           # гейты: выключенная интеграция с УК → пустой тик,
                           # окно в сутки, пачка ≤5 за тик.
UK_INTENT_RECONCILE_INTERVAL_MS=60000   # Тик прохода (зажат в [10000, 600000]).
UK_INTENT_RECONCILE_MAX_AGE_HOURS=24    # Окно (зажато в [1, 168]). Старше —
                           # не воскрешать: заявка по суточной аварии УК уже не
                           # поможет, а поток тикетов после долгого простоя вреден.
