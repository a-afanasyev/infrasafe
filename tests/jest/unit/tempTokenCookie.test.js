 ...
 *
 * Коды — через expand: старый JS делает `data.recoveryCodes.join('\n')` и
 * упал бы с TypeError, исчезни поле из ответа `setup-2fa`. Поэтому здесь они
 * возвращаются В ОБОИХ местах, фронт переезжает на `confirm-2fa`, а из ответа
 * `setup-2fa` поле убирает отдельный PR — когда новый JS разойдётся по всем
 * браузерам. Тот же expand/contract, что и у миграций.
 */
