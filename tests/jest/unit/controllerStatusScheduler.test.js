 ...
 *
 * Логика «молчит больше 10 минут → offline» написана давно, но её НИКТО не
 * вызывает по расписанию. Единственный вход — admin-эндпоинт
 * `POST /api/controllers/update-status`, то есть кнопка, которую надо нажать
 * руками.
 *
 * ⚠️ Поправка (12.08.2026). Здесь стояло «корректна и покрыта тестами» — не
 * так: сам запрос писал в несуществующий столбец и падал при каждом вызове.
 * Мок БД в юнит-тестах принимал любое имя столбца, поэтому поломка дожила до
 * первого тика этого планировщика на проде. Сам пересчёт теперь проверяется
 * на живой схеме в tests/jest/db/controllerStatus.db.test.js; здесь остаётся
 * только поведение самого планировщика.
 *
