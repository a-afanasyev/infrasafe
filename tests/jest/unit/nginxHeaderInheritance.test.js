...
describe('[CO-11] ни один location не теряет заголовки безопасности молча', () => {
    ...
    // [FOUNTAIN-CSP] Тут стоял сторож политики панели фонтана. Панель снята с
    // публикации 20.09.2026 — страницы, которую эта политика защищала, периметр
    // больше не отдаёт, и проверять нечего. Сторож не ослаблен, а заменён на
    // более сильный: tests/jest/unit/fountainControlSurface.test.js требует,
    // чтобы к контроллеру не вело НИ ОДНОЙ локации. Если панель вернут, вместе
    // с ней должна вернуться и проверка хешей — см. историю этого файла.
